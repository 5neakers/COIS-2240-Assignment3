...
public class RentalSystem {
	...
	private void loadData() {
		// method for loading the actual customer data from the txt file. Similar to all
		// 3 below need to make sure to actually split and trim based on format that it
		// was stored in
		...
		if (vFile.exists()) {
			try (Scanner scanner = new Scanner(vFile)) {
				while (scanner.hasNextLine()) {
					String line = scanner.nextLine();
					String[] p = line.split(",");
					// below is selecting the individual "parts" that correspond to the split by
					// commas in how it is stored in the actual .txt file
					String type = p[0];
					String plate = p[1];
					String make = p[2];
					String model = p[3];
					...
					Vehicle v = null;
					if (type.equals("Car"))
						v = new Car(make, model, year, Integer.parseInt(p[6]));
					else if (type.equals("Minibus"))
						v = new Minibus(make, model, year, Boolean.parseBoolean(p[6]));
					else if (type.equals("PickupTruck"))
						v = new PickupTruck(make, model, year, Double.parseDouble(p[6]), Boolean.parseBoolean(p[7]));
					else if (type.equals("SportCar"))
						v = new SportCar(make, model, year, Integer.parseInt(p[6]), Integer.parseInt(p[7]),
		...
		if (rFile.exists()) {
			try (Scanner scanner = new Scanner(rFile)) {
				while (scanner.hasNextLine()) {
					String line = scanner.nextLine();
					// given the format its stored in we need to split and trim this so its usable
					// by the program
					String[] parts = line.split("\\|");
...
//note added some changes in this commit to the save and additionally a new helper method briefly described below to ensure the type of vehicle is being considered and respective varialbes 
	private void saveVehicle(Vehicle v) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter("vehicles.txt", true))) {
			// I made some modifications to below for task 2 that now factors in the type of
			// vehicle properly as was ignoring that before
			String extra = "";
			if (v instanceof SportCar) {
				SportCar s = (SportCar) v;
				extra = "," + s.getNumSeats() + ",0,false"; // below adjusts for type of vehicle properly and what needs
															// to be stored in txt
	...
	public boolean addVehicle(Vehicle vehicle) {
		// standard if statement and using existing methods to see if the plate already
		// exists
	...
	public Customer findCustomerById(int id) {
		for (Customer c : customers) {
			if (c.toString().contains(String.valueOf(id)))
				return c;
		}
		return null;
	}

	// modified below once again as now is included in our test cases
	public boolean rentVehicle(Vehicle v, Customer c, LocalDate date, double amount) {
		if (v instanceof Rentable) {
			// below is specific if statement to determine if vehicle has already been
			// rented or not
...
//note I originally had a helper method here but it was causing me issues. Made simpler for code but was getting errors with it after modifying to bool so simply removed and incorporated in other methods 
	public boolean returnVehicle(Vehicle v, Customer c, LocalDate date, double fees) {
		if (v instanceof Rentable) {
			if (v.getStatus() != Vehicle.VehicleStatus.Rented) {
				System.out.println("Cannot return the vehicle as its already rented!");
				return false; // if returns false then tests will fail as plate already exists
			}
...
//removed the actual helper and just put into here 
			try (java.io.BufferedWriter writer = new java.io.BufferedWriter(new java.io.FileWriter("vehicles.txt"))) {
				for (Vehicle vehicle : vehicles) {
					writer.write(vehicle.getLicensePlate() + "," + vehicle.getMake() + "," + vehicle.getModel() + ","
			...
			return true; // one case where test is true and
		}
		return false;
	}
	...
	public void displayVehicles(Vehicle.VehicleStatus status) {
		...
		for (Vehicle v : vehicles) {
			if (v.getStatus() == status) {
				System.out.printf("|%-16s | %-12s | %-12s | %-12s | %-6d | %-18s |%n", v.getClass().getSimpleName(),
